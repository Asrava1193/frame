...
public class SeleniumReport extends Report {
	...
	protected void takeScreenshot(String screenshotPath) {
		...
		case API:
			break;
		case HEALENIUM:
			SelfHealingDriver SHdriver = driver.getSelfHealingDriver();
			 //scrFile = ((TakesScreenshot) ((WebDriver)SHdriver)).getScreenshotAs(OutputType.FILE);
			scrFile = ((TakesScreenshot) SHdriver.getDelegate()).getScreenshotAs(OutputType.FILE);
			break;
		case LOCAL:
		case GRID:
		case MOBILE:
			...
			try {
				if ("RemoteWebDriver".equals(driver.getWebDriver().getClass().getSimpleName())) {
					...
					scrFile = ((TakesScreenshot) augmentedDriver).getScreenshotAs(OutputType.FILE);
						...
						scrFile = ((TakesScreenshot) driver.getWebDriver()).getScreenshotAs(OutputType.FILE);//scrFile = ((TakesScreenshot) driver.getWebDriver()).getScreenshotAs(OutputType.FILE);​
